...
let contractAddress = "0x621936D3b5EC46C56A1b8841AF30667ac34a692A";  // Replace with your actual contract address
let abi = [
  {
    "inputs": [
      {
...
async function updateContractInfo() {
  try {
    ...
    document.getElementById("balance").textContent = `Contract Balance: ${balanceInEth} ETH`;
...
async function addBeneficiary() {
  ...
  try {
      ...
      updateStatus(`Beneficiary added successfully with ${sharePercent}% share!`, "success");
        ...
        updateStatus(`Failed to add beneficiary: ${error.message || "Unknown error"}`, "error");
    }
  }
  ...
  async function depositFunds() {
    ...
    try {
        ...
        updateStatus(`Depositing ${amount} ETH to contract...`, "info");
        const tx = await contract.depositFunds({ value: amountInWei });
        
        updateStatus("Transaction sent, waiting for confirmation...", "info");
        await tx.wait();
        
        updateStatus(`Successfully deposited ${amount} ETH to the contract!`, "success");
